function Control(events, comms, connections, fans, DB){
	...
	Control.events.on('AuditDoorOpened', function(){
		...
		if(Control.states.domes==0 && Control.states.emo==0 && (Control.states.lightingScene==null || Control.states.lightingScene=='LightingScene1' || Control.states.lightingScene=='B/O')){
			PerformAction('LightingScene2');
		}
	})
	
	Control.events.on('SetInitialLightingPosition', function(){
		Control.states.lightingScene = 'B/O';
	});
